...
AUTHOR.        Duma.
*>----------------------------------------------------------------------
*> Objetivo: Gerar arquivo perfil_usuario a partir do arquivo texto
*>
*> Entrada...: perfil_usuario.txt
*>
*> Saida.....: perfil_usuario.dat
*>
*>----------------------------------------------------------------------
...
COPY "../share/perfil_usuario_se.cpy".
...
COPY "../share/perfil_usuario_fd.cpy".

FD  ARQUIVO_TXT
    VALUE OF FILE-ID IS  "../txt/perfil_usuario.txt".
01  REG-TXT                       PIC  X(08).
...
01  WK-TEL01-1                    PIC  X(057) VALUE
    "GERAR ARQ PRF. DO USUARIO A PARTIR DO ARQUIVO TXT".
01  WK-TEL01-2.
    03  WK-TEL-DATA.
...
010-INICIALIZA.
    ...
    OPEN I-O PERFIL_USUARIO
    IF FSL-OK
       MOVE "ERRO! ARQUIVO JA GERADO. O programa sera encerrado" TO WK-MSG
       DISPLAY SS-MSG
       STOP RUN
    END-IF
    OPEN OUTPUT PERFIL_USUARIO
    CLOSE  PERFIL_USUARIO
    OPEN I-O PERFIL_USUARIO
             ...
             NOT AT END
                ...
                IF REG-TXT(1:5)<>SPACES
                    ...
                    INVALID KEY 
                        ...
                        MOVE ST-PFU TO WK-MSG(61:02)
                        DISPLAY SS-MSG
                        CLOSE ARQUIVO_TXT PERFIL_USUARIO
                        STOP RUN
                    NOT INVALID KEY
                        display WK-LIDO line 02 column 08
